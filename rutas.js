...
//=====(solo admin)=====//
// guardar usuario
app.post("/api/usuarios/Guardar",security.SoloAdmin, function (request, response) {
    usuariosController.Guardar(request, response)
})

// cargar todas 
app.get("/api/usuarios/CargarTodas",security.SoloAdmin,  function (request, response) {
    usuariosController.CargarTodas(request, response)
    
})

// cargar por id
app.get("/api/usuarios/CargarId/:_id",security.SoloAdmin, function (request, response) {
    usuariosController.CargarId(request, response)
})

// actualizar usuario
app.put("/api/usuarios/Actualizar",security.SoloAdmin, function (request, response) {
    usuariosController.Actualizar(request, response)
})

// eliminar usuario
app.delete("/api/usuarios/Eliminar",security.SoloAdmin, function (request, response) {
...
app.post("/api/usuarios/SolicitudRecuperarPass", function (request, response) {
...
app.post("/api/productos/Guardar",security.SoloAdmin, function (request, response) {
    productosController.Guardar(request, response)
})

app.get("/api/productos/CargarTodas", security.SoloAdmin, function (request, response) {
...
app.put("/api/productos/Actualizar",security.SoloAdmin, function (request, response) {
    productosController.Actualizar(request, response)
})

app.delete("/api/productos/Eliminar",security.SoloAdmin, function (request, response) {
...
exports.CargarTodas = function(req, res) {
    Producto.find({}, (err, result) => {
        if (err) {
            res.status(500).json({ error: 'Error al cargar productos', detalles: err });
        } else {
